...
export function AnytimeView({ onOpenSearch, focusTaskId, onFocusHandled }: AnytimeViewProps) {
  ...
  const { tasks, loading, error, createTask, toggleTask, updateTask, reorderTasks, deleteTask } = useTasks('anytime')
  ...
  return (
    <div className="things-screen flex flex-col h-screen">
      {/* Navigation */}
      <NavigationBar title="Anytime" onSearch={onOpenSearch || (() => setIsSearchOpen(prev => !prev))} />
      ...
      <AnimatePresence>
        {isSearchOpen && (
          <SearchBar
            ...
            placeholder="Search anytime tasks..."
          />
        )}
      </AnimatePresence>
          ...
          <div className="flex items-center justify-center h-64 px-4">
            <div className="things-empty-card text-center px-8 py-8 rounded-[24px] max-w-sm w-full">
              ...
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">
                {searchQuery ? 'No matching tasks' : 'Your Anytime list is empty'}
              </h2>
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-6">
                {searchQuery ? 'Try a different search term' : 'This is your inbox for unscheduled tasks'}
              </p>
            </div>
          </div>
        ) : (
      ...
      {/* Quick Entry */}
      <QuickEntry onAddTask={handleAddTask} defaultStatus="anytime" />
